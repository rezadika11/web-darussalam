...
a("#export-container-0").is(":visible")&&!a(this).hasClass("loaded")&&a("#export_settings_0").load(ajaxurl+"?action=ai_ajax_backend&export=0&ai_check="+ai_nonce,function(d,f,c){"error"==f?a("#ai-error-container").text(ai_admin.error+" "+c.status+": "+c.statusText).show():(a("#export_settings_0").attr("name","export_settings_0"),a("#export-switch-0").addClass("loaded"))})});a("#load-settings-0").change(function(d){d=d.target.files[0];var f=new FileReader;f.readAsText(d,"UTF-8");f.onload=function(c){c=
c.target.result;a("#export_settings_0").val(c);jQuery("#load-settings-0").val("")}});a("#save-settings-0").click(function(){var d={action:"ai_ajax_backend","export":0,file:1,ai_check:ai_nonce},f=document.createElement("form");f.setAttribute("method","get");f.setAttribute("action",ajaxurl);for(var c in d)if(d.hasOwnProperty(c)){var e=document.createElement("input");e.type="hidden";e.name=c;e.value=encodeURI(d[c]);f.appendChild(e)}document.body.appendChild(f);f.submit();document.body.removeChild(f)});
